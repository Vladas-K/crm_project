from django.urls import path

from .views import (
    AnalyticsView,
    CalendarView,
    ClientCreateView,
    ClientDeleteView,
    ClientUpdateView,
    ClientListView,
    ClientAutocompleteView,
    DashboardView,
    EventCommunicationCreateView,
    EventCommunicationUpdateView,
    EventCreateView,
    EventDeleteView,
    EventDetailView,
    EventDocumentCreateView,
    EventDocumentUpdateView,
    EventExpenseCreateView,
    EventExpenseDeleteView,
    EventExpenseUpdateView,
    EventRiskCreateView,
    EventRiskDeleteView,
    EventRiskUpdateView,
    EventOutcomeCreateView,
    EventOutcomeDeleteView,
    EventOutcomeUpdateView,
    EventFormatListView,
    EventFormatCreateView,
    EventFormatDeleteView,
    EventFormatUpdateView,
    EventListView,
    EventAutocompleteView,
    EventUpdateView,
    EventVendorCreateView,
    EventVendorStatusUpdateView,
    EventVendorUpdateView,
    LeadListView,
    LeadAutocompleteView,
    LeadCreateView,
    LeadDeleteView,
    LeadUpdateView,
    PackageListView,
    PipelineView,
    PipelineStageCreateView,
    PipelineStageDeleteView,
    PipelineStageUpdateView,
    ServicePackageCreateView,
    ServicePackageDeleteView,
    ServicePackageUpdateView,
    TaskCreateView,
    TaskDeleteView,
    TaskDetailView,
    TaskListView,
    TaskStatusUpdateView,
    TaskUpdateView,
    EventTimelineItemCreateView,
    EventTimelineItemDeleteView,
    EventTimelineItemUpdateView,
    TeamView,
    VendorListView,
    VendorAutocompleteView,
    VendorCreateView,
    VendorDeleteView,
    VendorUpdateView,
)

app_name = "core"

urlpatterns = [
    path("", DashboardView.as_view(), name="dashboard"),
    path("leads/", LeadListView.as_view(), name="leads"),
    path("leads/autocomplete/", LeadAutocompleteView.as_view(), name="lead_autocomplete"),
    path("leads/create/", LeadCreateView.as_view(), name="lead_create"),
    path("leads/<int:pk>/edit/", LeadUpdateView.as_view(), name="lead_update"),
    path("leads/<int:pk>/delete/", LeadDeleteView.as_view(), name="lead_delete"),
    path("pipeline/", PipelineView.as_view(), name="pipeline"),
    path("pipeline/create/", PipelineStageCreateView.as_view(), name="pipeline_create"),
    path("pipeline/<int:pk>/edit/", PipelineStageUpdateView.as_view(), name="pipeline_update"),
    path("pipeline/<int:pk>/delete/", PipelineStageDeleteView.as_view(), name="pipeline_delete"),
    path("clients/", ClientListView.as_view(), name="clients"),
    path("clients/autocomplete/", ClientAutocompleteView.as_view(), name="client_autocomplete"),
    path("clients/create/", ClientCreateView.as_view(), name="client_create"),
    path("clients/<int:pk>/edit/", ClientUpdateView.as_view(), name="client_update"),
    path("clients/<int:pk>/delete/", ClientDeleteView.as_view(), name="client_delete"),
    path("events/", EventListView.as_view(), name="events"),
    path("events/autocomplete/", EventAutocompleteView.as_view(), name="event_autocomplete"),
    path("events/<int:pk>/", EventDetailView.as_view(), name="event_detail"),
    path("events/<int:event_pk>/tasks/create/", TaskCreateView.as_view(), name="event_task_create"),
    path(
        "events/<int:event_pk>/timeline/create/",
        EventTimelineItemCreateView.as_view(),
        name="event_timeline_create",
    ),
    path("events/<int:event_pk>/risks/create/", EventRiskCreateView.as_view(), name="event_risk_create"),
    path("events/<int:event_pk>/outcome/create/", EventOutcomeCreateView.as_view(), name="event_outcome_create"),
    path("events/<int:event_pk>/expenses/create/", EventExpenseCreateView.as_view(), name="event_expense_create"),
    path("events/<int:event_pk>/vendors/create/", EventVendorCreateView.as_view(), name="event_vendor_create"),
    path(
        "events/<int:event_pk>/communications/create/",
        EventCommunicationCreateView.as_view(),
        name="event_communication_create",
    ),
    path("events/<int:event_pk>/documents/create/", EventDocumentCreateView.as_view(), name="event_document_create"),
    path("tasks/", TaskListView.as_view(), name="tasks"),
    path("tasks/create/", TaskCreateView.as_view(), name="task_create"),
    path("tasks/<int:pk>/", TaskDetailView.as_view(), name="task_detail"),
    path("tasks/<int:pk>/edit/", TaskUpdateView.as_view(), name="task_update"),
    path("timeline/<int:pk>/edit/", EventTimelineItemUpdateView.as_view(), name="event_timeline_update"),
    path("timeline/<int:pk>/delete/", EventTimelineItemDeleteView.as_view(), name="event_timeline_delete"),
    path("risks/<int:pk>/edit/", EventRiskUpdateView.as_view(), name="event_risk_update"),
    path("risks/<int:pk>/delete/", EventRiskDeleteView.as_view(), name="event_risk_delete"),
    path("outcomes/<int:pk>/edit/", EventOutcomeUpdateView.as_view(), name="event_outcome_update"),
    path("outcomes/<int:pk>/delete/", EventOutcomeDeleteView.as_view(), name="event_outcome_delete"),
    path("tasks/<int:pk>/status/", TaskStatusUpdateView.as_view(), name="task_status_update"),
    path("tasks/<int:pk>/delete/", TaskDeleteView.as_view(), name="task_delete"),
    path("expenses/<int:pk>/edit/", EventExpenseUpdateView.as_view(), name="event_expense_update"),
    path("expenses/<int:pk>/delete/", EventExpenseDeleteView.as_view(), name="event_expense_delete"),
    path("event-vendors/<int:pk>/edit/", EventVendorUpdateView.as_view(), name="event_vendor_update"),
    path("event-vendors/<int:pk>/status/", EventVendorStatusUpdateView.as_view(), name="event_vendor_status_update"),
    path(
        "communications/<int:pk>/edit/",
        EventCommunicationUpdateView.as_view(),
        name="event_communication_update",
    ),
    path("documents/<int:pk>/edit/", EventDocumentUpdateView.as_view(), name="event_document_update"),
    path("events/create/", EventCreateView.as_view(), name="event_create"),
    path("events/<int:pk>/edit/", EventUpdateView.as_view(), name="event_update"),
    path("events/<int:pk>/delete/", EventDeleteView.as_view(), name="event_delete"),
    path("formats/", EventFormatListView.as_view(), name="formats"),
    path("formats/create/", EventFormatCreateView.as_view(), name="format_create"),
    path("formats/<int:pk>/edit/", EventFormatUpdateView.as_view(), name="format_update"),
    path("formats/<int:pk>/delete/", EventFormatDeleteView.as_view(), name="format_delete"),
    path("vendors/", VendorListView.as_view(), name="vendors"),
    path("vendors/autocomplete/", VendorAutocompleteView.as_view(), name="vendor_autocomplete"),
    path("vendors/create/", VendorCreateView.as_view(), name="vendor_create"),
    path("vendors/<int:pk>/edit/", VendorUpdateView.as_view(), name="vendor_update"),
    path("vendors/<int:pk>/delete/", VendorDeleteView.as_view(), name="vendor_delete"),
    path("packages/", PackageListView.as_view(), name="packages"),
    path("packages/create/", ServicePackageCreateView.as_view(), name="package_create"),
    path("packages/<int:pk>/edit/", ServicePackageUpdateView.as_view(), name="package_update"),
    path("packages/<int:pk>/delete/", ServicePackageDeleteView.as_view(), name="package_delete"),
    path("calendar/", CalendarView.as_view(), name="calendar"),
    path("analytics/", AnalyticsView.as_view(), name="analytics"),
    path("team/", TeamView.as_view(), name="team"),
]
